...
EXAMPLES = [
    ...
    {
        "kind": "feature",
        "name": "Zero-Copy iGPU/NPU Buffers",
        "path": "zero_copy",
        "datatypes": "f32",
        "description": (
            "Buffers the iGPU and the NPU can both address, so a tensor handed "
            "from one to the other never moves -- with a benchmark of the "
            "hand-off with and without copies, under both NPU runtimes."
        ),
    },
    {
        "kind": "model",
...
def generate_kind_table(kind, base_url=""):
    """Generate markdown table rows for one non-operator section."""
    rows = []
    for ex in EXAMPLES:
        if ex.get("kind") != kind:
            continue
...
def generate_readme(base_url=""):
    ...
    table_body = "\n".join(table_rows)
    model_body = "\n".join(generate_kind_table("model", base_url=base_url))
    feature_body = "\n".join(generate_kind_table("feature", base_url=base_url))
