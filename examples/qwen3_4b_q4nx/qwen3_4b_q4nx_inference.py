...
Qwen3-4B is the first of the Q4NX examples that is not Llama-shaped, and the
first that needed its own forward (`../llm_q4nx/qwen3_prefill.py`): each
head's 128 lanes are RMS-normalized between the QKV projection and RoPE. It is
also the first with DQ != D -- 32 heads of 128 against a 2560 model dim -- so
o_proj contracts 4096 -> 2560 rather than being square.
