...
module attributes {transform.with_named_sequence} {
  transform.named_sequence @__transform_main(%arg1: !transform.any_op {transform.readonly}) {
    ...
    // The row reduction stays f32. examples/weighted_rms_norm casts it to bf16
    // and is only exercised at N=256; at Llama's N=2048 a bf16 sum of 2048
    // positive terms undercounts systematically -- the accumulator outgrows
    // the tail -- and the resulting rstd was ~9% high, a visible per-row gain
    // error on every one of the model's 33 norms.
    //
    // f32 here is not the unsupported case from CLAUDE.md's type table: the
    // multiplies below are still cast to bf16, so this is bf16 products with
    // f32 accumulation, which is what mlir-air's own rms_residual.cc does
