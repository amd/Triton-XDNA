...
"""End to end: a Triton prefill feeding mlir-air's Q4NX fused decode.
    ...
    python gemma3_4b_q4nx_inference.py --backend npu --max-tokens 20
    python gemma3_4b_q4nx_inference.py --prefill-only     # no decode build needed
...
def main(argv=None):
    from gemma3_prefill import Gemma3Prefill
...
if __name__ == "__main__":
    ...
    raise SystemExit(
        harness.run(
            registry.spec(config.MODEL_NAME), config, Gemma3Prefill, doc=__doc__
        )
    )
